...
Il risultato che dimostriamo in questa sezione è stato il primo a mostrare il
legame tra approssimazione e verifiche probabilistiche e segna l'inizio della
moderna teoria dell'inapprossimabilità.

Definiamo dunque il problema della cricca massima e mostriamo, grazie al
...
\begin{proof}
	...
	Il nostro obiettivo sarà trasformare istanze di $L$ in grafi in modo tale
	che istanze positive vengano mappate in grafi in cui la cricca massima è
	abbastanza grande e istanze negativa vengano mappate in grafi in cui tutte
	le cricche sono piccolissime. Il fattore di approssimazione $\bar\varepsilon$,
	allora, sarà sufficiente a distinguere i due scenari.

	Dato input $x \in \{0, 1\}^n$, il verificatore legge una stringa casuale
	$r \in \{0, 1\}^{c \log n}$ e $q$ posizioni della dimostrazione $\pi$.
	...
	\begin{definition}
		...
		uno \emph{scenario accettante} se:
		\begin{itemize}
...
\hline
	...
	Sia $M = 2^{c \log n} = n^c$ il numero totale di possibili stringhe casuali.
	Costruiamo il grafo $G' = (V', E')$ nel modo seguente:
	\begin{itemize}
		\item \textbf{Vertici}: $V'$ è l'insieme di tutte le \emph{trascrizioni accettanti}, ossia le coppie $\langle R, \alpha \rangle$ con $R \in \{0, 1\}^{c \log n}$ e $\alpha \in \{0, 1\}^q$ tali che $V$, con stringa casuale $R$, interroga $q$ posizioni della dimostrazione e accetta quando i bit letti sono $\alpha$. Poiché vi sono $n^c$ stringhe casuali e per ciascuna al più $2^q$ risposte $\alpha$, il numero di vertici soddisfa $|V'| \le n^c \cdot 2^q = O(n^c)$. Il grafo $G'$ ha dunque dimensione polinomiale ed è costruibile in tempo polinomiale.
		\item \textbf{Archi}: due trascrizioni accettanti distinte $u = \langle R_1, \alpha \rangle$ e $v = \langle R_2, \beta \rangle$ sono collegate da un arco in $E'$ se e solo se sono \emph{compatibili}, ossia $R_1 \neq R_2$ e non discordano su alcuna posizione della dimostrazione comune (cioè, se $V$ su $R_1$ e su $R_2$ interroga la stessa posizione, $\alpha$ e $\beta$ vi assegnano lo stesso valore di bit).
	\end{itemize}

	Mostriamo che la dimensione della massima cricca in $G'$ soddisfa:
	\[
		\omega(G') = \max_{\pi} \bigl|\{ R \in \{0, 1\}^{c \log n} : V \text{ accetta } \pi \text{ su stringa casuale } R \}\bigr|.
	\]
	\begin{itemize}
		\item ($\ge$) Data una qualunque dimostrazione $\pi$, l'insieme delle trascrizioni $\{\langle R, \pi|_R \rangle : V \text{ accetta } \pi \text{ su } R\}$ (dove $\pi|_R$ denota la restrizione di $\pi$ ai $q$ bit interrogati sotto $R$) forma una cricca in $G'$, in quanto tutte le trascrizioni provengono dalla medesima dimostrazione $\pi$ (quindi sono mutualmente compatibili) e hanno stringhe casuali distinte. La cardinalità di questa cricca è pari al numero di stringhe casuali su cui $V$ accetta $\pi$.
		\item ($\le$) Viceversa, sia $K \subseteq V'$ una cricca in $G'$. I vertici di $K$ hanno stringhe casuali a due a due distinte e sono a due a due compatibili; definiscono pertanto un'unica dimostrazione parziale $\tilde{\pi}_K$. Estendendo arbitrariamente $\tilde{\pi}_K$ a una dimostrazione completa $\pi$, il verificatore $V$ accetta $\pi$ su tutte le $|K|$ stringhe casuali associate ai vertici di $K$. Ne deriva che $|K| \le \max_\pi |\{R : V \text{ accetta } \pi \text{ su } R\}|$.
	\end{itemize}

	Dalla caratterizzazione di $\omega(G')$ segue che:
	\begin{itemize}
		\item se $x \in L$, per completezza esiste una dimostrazione $\pi$ accettata su tutte le $M = n^c$ stringhe casuali, per cui $\omega(G') = n^c$;
		\item se $x \notin L$, per robustezza ogni dimostrazione $\pi$ è accettata su al più $\delta n^c < \varepsilon n^c$ stringhe casuali, per cui $\omega(G') < \varepsilon n^c$.
	\end{itemize}

	Se esistesse un algoritmo di $\varepsilon$-approssimazione efficiente per \texttt{MAX-CLIQUE}, esso restituirebbe su input $G'$ una cricca $K$ tale che:
	\begin{itemize}
		\item se $x \in L$, $|K| \ge \varepsilon \cdot \omega(G') = \varepsilon n^c$;
		\item se $x \notin L$, $|K| \le \omega(G') < \varepsilon n^c$.
	\end{itemize}
	Controllando se $|K| \ge \varepsilon n^c$, potremmo quindi decidere in tempo polinomiale se $x \in L$, il che implicherebbe $\P = \NP$.
\end{proof}
