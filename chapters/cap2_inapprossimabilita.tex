...
cioè ottenerne una controparte deterministtica.}.
...
Come immediata conseguenza, per ogni $\varepsilon > \frac12$ non esiste una
$\varepsilon$-approssimazione di $q\CSP_2$ a meno che $\P = \NP$.

% Supponiamo infatti un tale algoritmo di approssimazione esista.
% Questo potrebbe essere utilizzato per distinguere lo scenario ``$x \in L$'',
% nel cui caso il valore della soluzione approssimata è almeno $\varepsilon$,
% e lo scenario ``$x \notin L$'', quando invece il valore della soluzione
% approssimata è al più $\frac12$.

Dimostriamo innanzitutto che il Teorema \ref{thm:pcp} è equivalente al
