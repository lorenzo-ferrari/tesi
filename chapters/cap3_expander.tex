...
parte, quanto basta per dimostrare il Teorema PCP e per migliorare il risultato
di inapprossimabilità di \texttt{MAX-CLIQUE}.

\section{Passeggiate aleatorie su grafi}

Un grafo si dice \emph{regolare di grado $d$}, d'ora in poi $d$-regolare, se
ogni nodo ha esattamente $d$ vicini. Per comodità, in questo capitolo
tratteremo solo grafi non diretti e $d$-regolari.
...
$\lambda_1 \ge \lambda_2 \ge \dots \lambda_n$.
...
Poiché gli autospazi di $A$ sono ortogonali, possiamo definire equivalentemente
$\lambda(G)$ in modo variazionale.
...
% \lorenzo{Un expander si comporta come un grafo casuale.}

In questa sezione presentiamo alcune delle proprietà pseudo-casuali degli
expander. Prima tra queste la velocità di convergenza alla distribuzione
uniforme.

\begin{theorem}{Convergenza alla distribuzione uniforme} \label{thm:expander-conv}
...
% Fissato $\lambda$, ciò significa che partendo da un qualunque nodo ed
% eseguendo una camminata aleatoria di $O(\log n)$ passi, il nodo di arrivo è
% distribuito quasi uniformemente. Nello specifico, un expander di $n$ nodi ha
% diametro\footnote{Cioè massima distanza tra due nodi.} $O(\log n)$.
% \lorenzo{Lo dico? Magari nomino addirittura il Teorema di Reingold? Sicuramente
% è interessante, forse però voglio parlare più o meno solo di PCP.}

\begin{proof}[Dimostrazione del Teorema~\ref{thm:expander-conv}]
...
\begin{proof}[Dimostrazione del Teorema~\ref{thm:expander-walk}]
	...
	scrive come $\lvert (B A)^{k - 1} B \one \rvert_1$, dove
	...
	da $B$.
	L'obiettivo ora è stimare la norma matriciale di $B A$, per cui ci sarà
	utile il seguente
	\begin{lemma}
	...
	\end{lemma}
	\begin{proof}
		Affinché la tesi valga, si deve necessariamente avere
		$C = \frac1{\lambda}\big( A - (1 - \lambda)J \big)$.
		Applichiamo la definizione variazionale di $\lambda$.
...
\begin{proof}
	...
	$x^\parallel := \frac1n \bm{1}$. Vale per definizione
	\[
		p = \sum_{v \in V}{ y_v (A^i x)_v }
		  = \langle y, A^i x \rangle.
	\]

	Osserviamo $\norm{x} ^2 \le \big(\sum_v{\lvert x_v \rvert} \big)
	\max_v{\norm{x_v}} = \max_v{\norm{x_v}}
